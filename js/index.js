const main = () => {
  $('.wiki-search').on('submit', event => {
    ...
    .done(result => {
      ...
      $('.result-list').hide().append(htmlList).fadeIn(700);
      $('.wiki-search__input').val('');
    })
    .fail(() => {
      $('.wiki-search__input').val('');
      $('.result-list').append(`<li class='result-list__error'>Please check your internet connection again!</li>`);
    })
  });
};
